...
if(isset($_GET['deal'])){
  ...
  $all = false;
  if ( $row === false ) {
      header('Location: deal_review.php');
      return;
  }

} else {
  $stmt = $pdo->prepare("SELECT deal.deal_id,deal_name,deal_status,deal_company,country_listvalidity,validity, reward, reward_unit
    FROM deal_review JOIN deal ON deal_review.deal_id = deal.deal_id WHERE deal_status = 'pending'");
  $stmt->execute();
  ...
  $id = $_SESSION['dealID'];
  $stmt = $pdo->prepare("UPDATE deal_review SET deal_status ='approved' WHERE deal_id='$id'");
  $stmt->execute();
  header('Location: deal_review.php');
...
if(isset($_POST['reject'])) {
  $id = $_SESSION['dealID'];
  $stmt = $pdo->prepare("UPDATE deal_review SET deal_status ='rejected' WHERE deal_id='$id'");
  $stmt->execute();
  header('Location: deal_review.php');
        ...
        <div class="row" style="height: 100%;">
          <div class="col-2 text-white" style="height: 100%; background-color: #CAF0F8; display: flex; flex-direction: column; justify-content: center;">
              ...
              <div class="d-flex align-items-start flex-column" style="width: 100%;">
                <button type="button" class="btn btn-light btn-lg btn-block mb-2" style="width: 100%;"onclick="window.location.href='#'">Deal Review</button>
          ...
          <div class="col-10">
              ...
              <div class="container-fluid">
                <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                  <div class="navbar-nav">
                    <a class="nav-link active fs-5 fw-bold" aria-current="page" href="deal_review.php">Home</a>
                    <?php
                    if(!$all){
            ...
            <div id="showdeals">
              <?php
                if($all){
                  foreach($rows as $row){
                    $id = "deal_review.php?deal=".$row['deal_id'];
                    ...
                    echo '<div class="card-footer text-muted">Valid until '.htmlentities($row['validity']).'</div></div>';
                    echo '</a>';
                    echo '<br/>';
                  }
                } else {
                    ...
                    echo '<p class="text-start card-text d-flex align-items-center">Tagline: <br/>'.htmlentities($row['tagline']).'</p></div>';
                    echo '<div class="bg-primary p-2 d-flex bd-highlight"><img src="https://image.flaticon.com/icons/png/128/3179/3179668.png" style="width: 30px; height:30px;" class="align-self-center"/><h4 class="align-self-center ms-2" style="color: white;">'.htmlentities($row['reward_unit'].$row['reward']).'</h4></div></div>';

                    echo '
                    <form action="" method="POST">
                    <div class="d-flex align-items-end flex-column bd-highlight mb-3 pe-3">
                    <input name="approve" id="approve" class="btn btn-success p-2 bd-highlight col-4 mb-2" type="submit" value="Approve">
                    <input name="reject" id="reject" class="btn btn-danger p-2 bd-highlight col-4 mb-2" type="submit" value="Reject">
                    </div>
                    </form>';
